...
# kiểm tra POST
#// Xác định nếu đã nhận được thông tin đăng nhập từ form
#// Tiếp theo, sử dụng hàm validate để làm sạch dữ liệu nhập vào.3
{
...
# Nếu thông tin đăng nhập không đầy đủ, chuyển hướng về trang đăng nhập và hiển thị thông báo lỗi.
    if(empty($user_name)) {
...
#Xác định cấp độ người dùng (User hoặc Staff):
        if ($user_level == 'User') {
            # Database Query (SQL)
        ...
        if (mysqli_num_rows($result) == 1) {
            ...
            if (password_verify($password, $row['password'])) {
                ...
                if(isset($_SESSION['user_name'])) {
                    ...
                    #Chuyển hướng và thiết lập phiên làm việc
                    exit();
                }
                else {
