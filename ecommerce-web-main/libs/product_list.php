        ...
        <div class="row mt-5">
            ...
            if (isset($_GET['submit_filter'])) {
                ...
                & $_GET['storage'] == 'all' & $_GET['min-price'] == '' & $_GET['max-price'] == '') {
                    $records = $db->getAllProducts(); #Trong trường hợp người dùng đã nhấn nút "Lọc", mã sẽ thực hiện lấy sản phẩm dựa trên các bộ lọc đã chọn. Nếu không, nó sẽ lấy tất cả sản phẩm
                } else {
                    $product_name = '';
            ...
            if (mysqli_num_rows($records) > 0) {
                while ($data = mysqli_fetch_assoc($records)) {
                    #Nếu có sản phẩm được tìm thấy trong cơ sở dữ liệu, mã sẽ duyệt qua từng dòng và lưu thông tin của sản phẩm vào một mảng ($product_data).

                    $product_data[] = $data;
                }
                ##Sau đó, vòng lặp foreach sẽ sử dụng thông tin này để hiển thị các card sản phẩm trên trang.
...
#Trong vòng lặp foreach, mỗi sản phẩm được hiển thị trong một card có hình ảnh, tên, SKU, và các thông số kỹ thuật như CPU, RAM, GPU, Storage.
Giá sản phẩm và nút "Thêm vào giỏ hàng" được hiển thị, và một form được sử dụng để gửi dữ liệu sản phẩm lên server khi người dùng thêm vào giỏ hàng.
Nếu không có sản phẩm nào được tìm thấy, sẽ hiển thị một thông báo "No products found!".